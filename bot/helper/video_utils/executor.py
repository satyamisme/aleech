...
class VidEcxecutor(FFProgress):
    ...
    async def _run_cmd(self, cmd, status='prog'):
        ...
        if self.listener.suproc == 'cancelled' or code == -9:
            ...
            LOGGER.error(f"FFmpeg error: {(await self.listener.suproc.stderr.read()).decode().strip()}")
            self._files.clear()
        return False
    ...
    async def _merge_preremove_audio(self):
        ...
        async with file_lock:
            ...
            for file in file_list:
                ...
                if selections == [s['index'] for s in streams if s['codec_type'] in ['video', 'audio', 'subtitle']]:
